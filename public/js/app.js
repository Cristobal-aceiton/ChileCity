    ...
    // ── Compatibilidad multi-página ──────────────────────────────────────────
    // El sitio se separó en un .html por apartado (ver CC_PAGE_MAP definido
    // inline en cada página, antes de este script). Si mostrarPantalla() pide
    // un id que no vive en el HTML de la página actual, en vez de fallar en
    ...
    async function goToDashboard(user, opts = {}) {
      ...
      // index.html. En el resto de páginas goToDashboard() se sigue llamando
      // al restaurar sesión (initSesion), pero no hay dónde pintar nombre o
      // avatar — cada asignación se protege con un chequeo de existencia.
      const elName = document.getElementById('discord-name');
      if (elName) elName.textContent = user.name;
      const elTag = document.getElementById('discord-tag');
      if (elTag) elTag.textContent = user.tag || '';
      const elAvatar = document.getElementById('discord-avatar');
      ...
      if (elProfileAvatar) elProfileAvatar.src = user.avatar;
    ...
    // NOTA multi-página: #f-fecha solo existe en registro-civil.html. En el
    // resto de páginas este bloque debe protegerse: si no, el error frenaba
    // la ejecución del resto del archivo (initSesion, listeners, etc. nunca
    // se registraban en ninguna otra página).
    const elFFecha = document.getElementById('f-fecha');
    if (elFFecha) elFFecha.max = new Date().toISOString().split('T')[0];
    ...
    // ── User pill ─────────────────────────────────────────────────────────────
    // NOTA multi-página: user-pill/logout-btn/portal-btn viven dentro de la
    // sección "dashboard", que ahora solo existe en dashboard.html/index.html.
    // Se protege cada listener para que el resto del script siga corriendo
    // igual en las páginas donde estos botones no existen.
    const elUserPill = document.getElementById('user-pill');
    if (elUserPill) {
      elUserPill.addEventListener('click', (e) => {
        if (e.target.closest('#logout-btn')) return;
        elUserPill.classList.toggle('open');
      });
      document.addEventListener('click', (e) => {
        if (!elUserPill.contains(e.target)) elUserPill.classList.remove('open');
      });
    }
    const elLogoutBtn = document.getElementById('logout-btn');
    if (elLogoutBtn) {
      elLogoutBtn.addEventListener('click', () => {
        if (elUserPill) elUserPill.classList.remove('open');
        goToLanding();
      });
    }
    const elPortalBtn = document.getElementById('portal-btn');
    if (elPortalBtn) {
      elPortalBtn.addEventListener('click', () => {
        irAlPortal();
      });
    }
    ...
    async function cargarTopRicosHero() {
      ...
      try {
        ...
        if (top3.length === 0) {
          lista.innerHTML = '<div class="top-hero-empty"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M8 21h8M12 17v4M7 4h10v4a5 5 0 0 1-10 0V4Z"/><path d="M7 5H3v2a4 4 0 0 0 4 4M17 5h4v2a4 4 0 0 1-4 4"/></svg><span>Aún no hay cuentas bancarias</span></div>';
          return;
        }
        ...
        lista.innerHTML = '<div class="top-hero-empty"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><circle cx="12" cy="12" r="9"/><path d="M12 8v5M12 16h.01"/></svg><span>Error al cargar</span></div>';
      }
    }
