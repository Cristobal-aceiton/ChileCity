    ...
    async function cargarEmpresas(page = 1) {
      ...
      try {
        const res = await fetch(`/api/empresas?action=listar&page=${page}&limit=${EMP_LIMIT}`);
    ...
    async function cargarAdminEmpresas(page = 1) {
      ...
      try {
        const res = await fetch(`/api/empresas?action=admin_listar&page=${page}&limit=${EMP_LIMIT}`);
        const data = await res.json();
        loading.style.display = 'none';
        if (!res.ok) {
    ...
    async function adminCrearEmpresa() {
      ...
      try {
        const res = await fetch('/api/empresas?action=admin_crear', {
          method: 'POST',
    ...
    async function adminGuardarEdicionEmpresa() {
      ...
      try {
        const res = await fetch('/api/empresas?action=admin_editar', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
    ...
    async function adminEliminarEmpresa(id) {
      ...
      try {
        await fetch(`/api/empresas?action=admin_eliminar&empresa_id=${id}`, { method: 'DELETE' });
